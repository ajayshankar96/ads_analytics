    ...
    <div>
      <div style={s.controls}>
        ...
        <select
          ...
          onChange={(e) => setSelectedAdv(Array.from(e.target.selectedOptions).map(o => o.value))}
        >
          {advOptions.map(a => <option key={a} value={a}>{a}</option>)}
        </select>
        {selectedAdv.length > 0 && (
          <button style={{ ...s.viewBtn, fontSize: 11 }} onClick={() => setSelectedAdv([])}>
            Clear ({selectedAdv.length})
          </button>
        )}
      </div>
